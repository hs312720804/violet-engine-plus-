type ELLoadingInstance = import('element-plus/lib/el-loading/src/loading.type').ILoadingInstance
// type ELFileResultHandler = import('element-plus/lib/el-upload/src/upload.type').FileResultHandler
// type ELFileHandler = import('element-plus/lib/el-upload/src/upload.type').FileHandler
type ElFile = import('element-plus/lib/el-upload/src/upload.type').ElFile
type ElUploadFile = import('element-plus/lib/el-upload/src/upload.type').UploadFile
type ELFormRulesMap = import('element-plus/lib/el-form/src/form.type').FormRulesMap
